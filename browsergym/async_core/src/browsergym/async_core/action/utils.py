...
async def get_elem_by_bid(
    page: playwright.async_api.Page, bid: str, scroll_into_view: bool = False
...
async def smooth_move_visual_cursor_to(
...
async def add_demo_mode_effects(
    ...
    if box:
        ...
        is_top_element = await check_for_overlay(page, bid, elem, box)
        ...
        if highlight_box:
            await highlight_by_box(page, box, color=color)


async def call_fun(fun: callable, retry_with_force: bool, sync=True):
    if sync:
        try:
            fun(force=False)
        ...
        try:
            await fun(force=False)
        except playwright.async_api.TimeoutError as e:
            if retry_with_force:
                await fun(force=True)
            else:
                raise e
