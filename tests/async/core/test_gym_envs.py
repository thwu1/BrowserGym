...
async def test_max_episode_steps():
    # no max_steps
    env = gym.make(
        "browsergym_async/openended",
        task_kwargs={"start_url": TEST_PAGE},
...
async def test_active_page():
    ...
    obs, reward, term, trunc, info = await env.step("await page.context.new_page()")

    assert len(obs["open_pages_urls"]) == 2
    assert obs["active_page_index"] == 1

    obs, reward, term, trunc, info = await env.step("await page.context.pages[0].mouse.click(5, 5)")

    assert len(obs["open_pages_urls"]) == 2
    assert obs["active_page_index"] == 0

    obs, reward, term, trunc, info = await env.step("await page.context.pages[1].mouse.click(5, 5)")

    assert len(obs["open_pages_urls"]) == 2
    assert obs["active_page_index"] == 1

    obs, reward, term, trunc, info = await env.step("await page.context.pages[1].close()")

    assert len(obs["open_pages_urls"]) == 1
    assert obs["active_page_index"] == 0

    obs, reward, term, trunc, info = await env.step("await page.close()")
...
async def test_nested_iframes_default_demo_mode():
    demo_mode = "default"
...
async def test_resizeable_window(resizeable_window, size):
    env = gym.make(
        "browsergym_async/openended",
        task_kwargs={"start_url": TEST_PAGE},
        headless=__HEADLESS,
